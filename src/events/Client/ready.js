...
module.exports = {
    ...
    run: async(_, client) => {
        ...
        setInterval(async () => {
            const connecteduser = await fetch('http://localhost:3333/instance/containers/connected');
            const data = await connecteduser.json();
            const connectedIds = data.data;
            const idstonumber = connectedIds.length;

            const allinstance = await fetch('http://localhost:3333/manager/status', {
                method: 'GET',
                headers: {
            ...
            const pingResponse = await fetch('http://localhost:3333/manager/ping', {
                method: 'GET',
                headers: {
                    'Accept': 'application/json'
                }
